...
BEGIN
  ...
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'feedback_tags') THEN
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS feedback_tags_name_form_idx ON feedback_tags (name, (form_id::text)) WHERE form_id IS NOT NULL;
    CREATE UNIQUE INDEX IF NOT EXISTS feedback_tags_global_name_idx ON feedback_tags (name) WHERE form_id IS NULL;
